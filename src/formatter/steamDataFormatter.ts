...
/**
 * ゲームのカテゴリ一覧をカンマ区切り文字列へ整形する
 * 
 * Steam詳細情報からカテゴリーを取得し、
 * 表示用の文字列として返す。
 * カテゴリー情報が存在しない場合は undefined を返す。
 * 
 * @param game Steamゲーム詳細情報
 * @returns カンマ区切りのジャンル文字列
 */
export function formatCategories(game: ExtendedSteamGameDetail): string | undefined {
    return game.steamDetail.categories?.map(x => x.description).join(", ");
}